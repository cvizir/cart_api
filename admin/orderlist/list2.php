<?php
	...
	$where=" WHERE `order_code` LIKE '%$sh_txt%' OR `invoice_name` LIKE '%$sh_txt%' OR `invoice_email` LIKE '%$sh_txt%'  OR `total_amount` LIKE '%$sh_txt%'OR `no` LIKE '%$sh_txt%' OR `order_state` LIKE '%$sh_txt%' OR `store_num` = '$sh_txt' ";	
	...
	$where=" WHERE `member_no` = '$sh2_txt' OR `member_code` = '$sh2_txt'";	
...
<?php 
	...
	checkAdmimPv($_SESSION["admin_pv"],$check_txt,$check_type,$trace);
              ...
              <tr>
                ...
                <tr>
                  ...
                  <td colspan="3">
                    ...
                    <form name="form1" method="post" action="order_tr.php">

                   訂單匯出： <select name="sh_state" id="sh_state"  >

                      <option value="" selected="selected"  <?php chkSelected('',$_REQUEST['sh_state']); ?> >全部</option>
...
<td align="left" class="tb_list_info"style="text-align:left;"><a href="https://shopping.windmill.com.tw/udfif_djsd/orderlist/email_order-R2.php?order_code=<?php echo $orderlist['order_code']; ?>&member_code=<?php echo $orderlist['member_code']; ?>" target="_blank">
